...
class UrlScrapperJob < ActiveJob::Base
    ...
    def request_uri
        warn "URI without request_uri: #{@uri}" if @uri.request_uri.nil?
        @uri.request_uri.nil? ? '/' : @uri.request_uri
    end

    def fetch_headers(limit = REDIRECT_LIMIT)
        http = connection(limit)
        response = http.request_head(request_uri)
        handle_response(response, limit)
    end

    def fetch_page(limit = REDIRECT_LIMIT)
        http = connection(limit)
        response = http.request_get(request_uri)
        page = handle_response(response, limit)
        page.body
    end
